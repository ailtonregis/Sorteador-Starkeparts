...
export async function readState() {
  return (await readStateRecord()).state;
}

export async function readStateRecord() {
  const { url, key } = settings();
  const response = await fetch(`${url}/rest/v1/app_state?id=eq.main&select=data,updated_at`, {
    headers: headers(key, { Accept: 'application/json' }),
  });
  if (!response.ok) {
    const detail = await response.text();
    throw new Error(`Supabase GET ${response.status}: ${detail}`);
  }
